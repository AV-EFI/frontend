...
Project 1 / `AVefi User Stories Kanban (ALL User Stories)` snapshot after the latest inventory refresh:
...
| AP1.2 | Datenakquise | Acquire additional data deliveries and further AV holdings. | Data acquisition/import, partner onboarding, source institutions. |
...
| AP2 | Metadatenmanagement | Metadata-model evolution, data-delivery support, stronger normdata/GND linkage, documentation. | Major future backlog source, but only after project-coordination confirmation. |
| AP2.1 | Fachlicher Teilbereich laut Antrag | Scope and deliverables require explicit project-coordination decision before GitHub derivation. | No issue creation is derived from this report. |
| AP2.2 | Ausweitung des Datenschemas | Extend AVefi schema for relationships between works, manifestations and items. | Schema/model implications should be routed only after confirmed scope. |
...
| AP2.5 | Dokumentation und Evaluation | Extend interface/data-model documentation. | Documentation/QA. |
...
| AP4.2 | Abgleich- und Synchronisationsalgorithmen ausbauen | Better matching/sync for works and related data structures. | Dedupe/match/merge/similarity issues; backend/data. |
...
This is a first-pass mapping. It is meant to guide coordinated backlog governance; it is not a completion claim.

| Context | Relevant APs | Representative issues | Suggested review action |
| --- | --- | --- | --- |
...
| `#217` and `#220` | `#217` closed on 2026-07-13; `#220` closed earlier. | No further action unless historical metadata normalization is desired. |
...
1. Board hygiene first:
   ...
   - Project 3 / `AVefi: Public Release Board` no longer has open issues after the 2026-07-06 transfer; only closed/historical Project 3 items remain for optional archival review.
...
5. Then continue issue review in AP order:
   - Phase 1 closed/implemented consolidation first.
