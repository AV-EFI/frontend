...
export function printMetric(details: {
  ...
  const { label, value, total, warnThreshold, failThreshold } = details;

  if (typeof total === 'number') {
    const ratio = pct(value, total);
    const severity = typeof warnThreshold === 'number'
      ? severityFromValue(ratio, warnThreshold, failThreshold)
      : 'OK';
    printLine(
      `${label}: ${value}/${total} (${formatPct(ratio)}) [${severity}]`,
    );
    return;
  }

  const severity = typeof warnThreshold === 'number'
    ? severityFromCount(value, warnThreshold, failThreshold)
    : 'OK';
  printLine(`${label}: ${value} [${severity}]`);
}
