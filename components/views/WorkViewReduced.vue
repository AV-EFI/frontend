        ...
        <div v-if="compareWith" class="grid grid-cols-2 gap-8">
            ...
            <div class="space-y-4">
                ...
                <table class="table table-xs table-zebra w-full">
                    <tbody>
                        <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('handle') }">
                            <td class="font-semibold w-1/3">efi</td>
                            <td>{{ data?.handle || data?.compound_record?._source?.handle || '—' }}</td>
                        </tr>
            
                        <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('alternativeTitles') }">
                        ...
                        <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('directors') }">
                            <td class="font-semibold">{{ $t('directors') }}</td>
                            <td>{{ directors || '—' }}</td>
                        </tr>

                        <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('productionYear') }">
                            <td class="font-semibold">{{ $t('year') }}</td>
                            <td>{{ productionYear || '—' }}</td>
                        </tr>

                        <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('productionCountry') }">
                            <td class="font-semibold">{{ $t('country') }}</td>
                            <td>{{ productionCountry || '—' }}</td>
                        </tr>

                        <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('workType') }">
                            <td class="font-semibold">{{ $t('facettype') }}</td>
                            <td>{{ translateValue(workType) }}</td>
                        </tr>

                        <template v-if="showDetailed">
                            <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('producers') }">
                                <td class="font-semibold">{{ $t('producers') }}</td>
                                <td>{{ producers || '—' }}</td>
                            </tr>

                            <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('castMembers') }">
                                <td class="font-semibold">{{ $t('castmembers') }}</td>
                                <td>{{ castMembers || '—' }}</td>
                            </tr>

                            <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('subjects') }">
                            ...
                            <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('genres') }">
                            ...
                            <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('languages') }">
                                <td class="font-semibold">{{ $t('has_language') }}</td>
                                <td>{{ languages.length ? translateArray(languages).join(', ') : '—' }}</td>
                            </tr>

                            <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('duration') }">
                                <td class="font-semibold">{{ $t('duration') }}</td>
                                <td>{{ duration || '—' }}</td>
                            </tr>

                            <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('workCategory') }">
                                <td class="font-semibold">{{ $t('category') }}</td>
                                <td>{{ translateValue(workCategory) }}</td>
                            </tr>

                            <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('dataProvider') }">
                                <td class="font-semibold">{{ $t('dataprovider') }}</td>
                                <td>{{ dataProvider || '—' }}</td>
                            </tr>

                            <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('manifestationCount') }">
                                <td class="font-semibold">{{ $t('manifestations') }}</td>
                                <td>{{ manifestationCount }}</td>
                            </tr>

                            <tr :class="{ 'bg-warning/60 dark:bg-error': highlightDiffs && isDifferent('itemCount') }">
