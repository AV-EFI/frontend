<template>
    <div class="col-span-full mt-4" role="region" :aria-label="$t('normdataOverview')">
        ...
        <section v-if="sections.work.entries.length" class="border border-base-300 rounded-lg p-3 mb-3" role="region"
            ...
            <header class="flex items-center justify-between mb-2 gap-2">
                ...
                <ViewsKibanaEmbed
                    ...
                    :handle="handle" :time-from="kibana.timeFrom || 'now-15y'" :time-to="kibana.timeTo || 'now'"
                    :aria-label="$t('openInKibana')" />
            </header>

            <ul class="space-y-1 text-sm">
                <li v-for="(row, i) in sections.work.entries" :key="'w-' + i"
                    class="flex flex-wrap items-baseline gap-x-2">
                    <span class="font-mono truncate" :title="row.path">{{ row.path }}</span>
                    <span>→</span>
                    <template v-if="row.type === 'normref'">
                        ...
                        <span v-if="row.value.note" class="opacity-70">&nbsp;· {{ row.value.note }}</span>
                    </template>
                    <template v-else-if="row.type === 'identifier'">
                        <span v-if="row.value.id_category">{{ row.value.id_category }}:</span>
                        <span v-if="row.value.id_value" class="font-mono">{{ row.value.id_value }}</span>
                        <span v-if="row.value.note" class="opacity-70">&nbsp;· {{ row.value.note }}</span>
                    </template>
                </li>
            </ul>
        ...
        <section v-if="sections.manifestations.entries.length" class="border border-base-300 rounded-lg p-3 mb-3"
            ...
            <ul class="space-y-1 text-sm">
                <li v-for="(row, i) in sections.manifestations.entries" :key="'m-' + i"
                    class="flex flex-wrap items-baseline gap-x-2">
                    <span class="font-mono truncate" :title="row.path">{{ row.path }}</span>
                    <span>→</span>
                    <template v-if="row.type === 'normref'">
                    ...
                    <template v-else-if="row.type === 'identifier'">
                        <span v-if="row.value.id_category">
                            {{ row.value.id_category }}:
                        </span>
                        <span v-if="row.value.id_value" class="font-mono">{{ row.value.id_value }}</span>
                        <span v-if="row.value.note" class="opacity-70">&nbsp;· {{ row.value.note }}</span>
                    </template>
        ...
        <section v-if="sections.items.entries.length" class="border border-base-300 rounded-lg p-3" role="region"
            ...
            <ul class="space-y-1 text-sm">
                <li v-for="(row, i) in sections.items.entries" :key="'i-' + i"
                    class="flex flex-wrap items-baseline gap-x-2">
                    <span class="font-mono truncate" :title="row.path">{{ row.path }}</span>
                    <span>→</span>
                    <template v-if="row.type === 'normref'">
        ...
        <p v-if="!hasAny" class="opacity-70 text-sm">
            {{ $t('noNormdataFound') }}
        </p>
    </div>
</template>
