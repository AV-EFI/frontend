        ...
        <div class="bg-base-200 dark:bg-base-300 md:[width:calc(40%+50px)]">
                    ...
                    <button type="button"
                        ...
                        <div class="flex flex-col justify-center leading-tight w-full">
                            <div class="w-full flex flex-row justify-between items-center mb-2">
                                <LazyMicroBadgeCategoryComp category="avefi:Manifestation" :dense="false" class="mr-2" />
                                <div class="badge badge-neutral badge-sm dark:text-black" v-if="allItemsEmpty(m)">
                            ...
                            <GlobalClipboardComp :display-text="m.handle" class="text-xs dark:hover:text-gray-400"
                                                 :aria-label="$t('copyManifestationHandle')" key-text="avefi:AVefiResource" :dark-bg="false" />
                            <div class="text-sm font-medium mt-0.5" tabindex="0"
                    ...
                    <transition name="fade-slide">
                        <div v-if="selectedIndex === i + currentPage * itemsPerPage"
                            ...
                            <ul class="space-y-2 pl-1">
                                <li v-for="(item, j) in getFilteredItems(m).slice(0, 3)" :key="j"
            ...
            <div class="flex justify-between items-center mt-2 p-2">
                <button type="button"
                        ...
                        :disabled="currentPage === 0" :aria-label="`${$t('prevPage')}: ${$t('manifestations')}`" @click="prevPage">
                    {{ $t('prevPage') }}
                </button>
                ...
                <button type="button"
                        class="btn btn-xs btn-outline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
                        :disabled="currentPage >= totalPages - 1" :aria-label="`${$t('nextPage')}: ${$t('manifestations')}`"
                        @click="nextPage">
                    {{ $t('nextPage') }}
                </button>
            </div>
        ...
        <transition name="fade-slide">
            <div v-if="selectedManifestation"
                    ...
                    <ul class="space-y-2">
                        <li v-for="(item, j) in paginatedItems" :key="j" class="p-2 bg-base-100 dark:bg-base-200 rounded-md"
                            ...
                            <div tabindex="0" class="flex items-start" :aria-label="`${$t('item')}: ${item.handle}`">
                                <GlobalClipboardComp :display-text="item.handle" key-text="avefi:AVefiResource"
                                                     class="text-left hover:text-gray-700 dark:hover:text-gray-300 my-2" :aria-label="$t('copyItemHandle')"
                                                     font-size="text-xs" />
                            </div>
                            <div class="divider my-0"></div>
                            <span class="flex justify-start text-left font-semibold">
                                {{ item?.has_record?.has_primary_title?.has_name }}</span>
                ...
                <div class="flex justify-between items-center mt-2 px-2 text-xs">
                    <button type="button"
                            class="btn btn-xs btn-outline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
                            :disabled="itemPage === 0" :aria-label="`${$t('prevPage')}: ${$t('items')}`" @click="prevItemPage">
                        {{ $t('prevPage') }}
                    </button>
