<template>
  <div class="relative w-full">
    ...
    <div ref="carouselRef" class="carousel carousel-center w-full bg-white dark:bg-base-200 rounded-box p-4 overflow-x-auto scroll-smooth">
        <div
        ...
        <figure class="w-full">
          <div v-if="item.imgSrc" class="relative w-full h-48 md:h-56 lg:h-64 rounded overflow-hidden bg-white dark:bg-base-200">
            <img
              :src="item.imgSrc"
              alt=""
              loading="lazy"
          ...
          <div v-else class="w-full h-48 md:h-56 lg:h-64 flex items-center justify-center bg-gray-100 dark:bg-base-200 rounded overflow-hidden">
            <img
          ...
          <figcaption v-if="item.imgSourceText" class="h-auto break-words text-xs text-gray-500 mt-2 px-2 dark:text-gray-400">
            {{ t('imageSource') }}: <a :href="item.imgSourceLink" target="_blank" class="underline">{{ item.imgSourceText }}</a>, {{ t('author') }}: {{ item.imgAuthor }} / {{ item.imgLicense }} ({{ item.imgLicenseLink }})
          </figcaption>
        </figure>
        <div class="p-4 flex flex-col flex-1 w-full bg-white dark:bg-base-200">
          ...
          <p class="text-gray-700 text-sm mb-2 dark:text-gray-300 md:!line-clamp-none" :class="{ 'line-clamp-4': !showFullText }">
            {{ t(item.description) }}
          </p>
          <a
            class="md:hidden link link-secondary mb-2 dark:link-info"
            @click="toggleText"
          >
          ...
          <div class="mt-auto">
            ...
            >
              {{ t(item.linkText) }}
              <Icon
                class="hidden md:inline-block ml-1"
