  ...
  <div class="w-full xs:max-w-xs md:w-full lg:max-w-4xl mx-auto p-4 backdrop-blur-sm rounded-lg" role="search"
    ...
    <ClientOnly>
      <template v-if="searchDataStore && searchDataStore.formData">
        ...
        <FormKit id="searchComp" v-model="searchDataStore.formData" type="form" :actions="false" name="searchComp"
          role="search" :aria-label="$t('searchForm')" @submit="redirectToSearchScreen">
          <!-- Query input + submit -->
          <div class="flex w-full mx-auto mb-4 relative" role="group" :aria-labelledby="'search-input-label'">
            <div class="flex-grow relative">
              <SearchQueryAutocomplete ref="qaRef" v-model="searchTerm" name="search"
                ...
                :no-results-text="$t('noSuggestionsFound')" :recent-searches="recentSearchesWithUrl" :autofocus="false"
                @submit="onSubmit" @clear="searchTerm = ''" @select="onMainSelect"
            ...
            <div class="ml-0">
              <button type="submit" class="!rounded-l-none !rounded-r-xl flex btn btn-primary lg:btn-lg h-[56px]"
                :class="{'btn-disabled opacity-50 cursor-not-allowed': !canSubmit}" :aria-label="$t('submitSearch')"
                @click="handleClick">
                {{ buttonText }}
              </button>
            </div>
          ...
          <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
            ...
            <div class="space-y-2">
              <div v-for="(filter, index) in facetFilters" :key="filter.uid"
                ...
                <!-- Facet select -->
                <FormKit v-model="filter.facet" type="select" :placeholder="$t('selectFacet') || 'Select Facet'"
                ...
                <div class="flex-1 relative" @mousedown.stop>
                  <div class="flex justify-items-start items-start gap-2">
                    ...
                    <FormKit :model-value="filter.valueDisplay" type="text"
                      ...
                      inner-class="dark:!bg-gray-950 dark:!text-white" :disabled="!filter.facet" autocomplete="off"
                      ...
                      @input="onValueInput(index, $event)" @focus="onValueFocus(index)" @blur="onValueBlur(index)"
                  ...
                  <div v-if="filter.showSuggestions && filter.suggestions.length"
                    ...
                    <div v-for="(s, si) in filter.suggestions.slice(0, 10)" :id="`facet-sugg-${filter.uid}-${si}`"
                      ...
                      <span v-if="s.count && s.count > 1" class="text-xs text-gray-500 dark:text-gray-400 shrink-0">
                        ({{ s.count }})
                      </span>
                    </div>
                  </div>
                ...
                <div class="w-full lg:w-auto flex justify-end lg:justify-start">
                  <button type="button" class="btn btn-outline btn-error p-2" :aria-label="$t('remove') || 'Remove'"
