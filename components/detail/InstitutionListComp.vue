    ...
    <div>
        <div class="container">
            <ais-instant-search :search-client="searchClient" :index-name="indexName" :show-loading-indicator="true"
                                :routing="true" :future="{preserveSharedStateOnUnmount: true, cleanUrlOnDispose: false}">
                    ...
                    <div class="drawer-content w-full flex flex-col items-center justify-center">
                        <div class="search-panel__results w-full py-2">
                                    ...
                                    <template #default="{ currentRefinement, isSearchStalled, refine }">
                                        <div
                                            ...
                                            <label
                                                ...
                                                for="input_0"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 16">
                                                    <path
                                            ...
                                            <input type="search"
                                                   ...
                                                   :value="currentRefinement" :placeholder="$t('searchplaceholder')"
                                                   @input="handleRefine(refine, $event.target.value)">
                                            <span :class="[!isSearchStalled ? 'hidden' : '','loading loading-spinner loading-sm']" />
                                        </div>
                                    </template>
                                </ais-search-box>
                            </div>
                            ...
                            <div class="mt-2 mb-2 w-full">
                                <button class="btn btn-primary lg:hidden" :title="$t('showFacetItems')"
                                        @click="$toggleFacetDrawerState">
                                    <Icon name="tabler:chevron-right" />&nbsp;{{ $t('showFacetItems') }}
                                </button>
                                <div class="w-full flex flex-col md:flex-row justify-between md:justify-between p-2 mb-2">
                                ...
                                <div class="w-full p-2 flex flex-col md:flex-row justify-between">
                                    ...
                                    <div class="w-full md:w-1/3 flex flex-col justify-end">
                                        <FormKit type="select" :label="$t('sorting')" :disabled="true" name="sort"
                                                 ...
                                                 input-class="bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 border border-gray-300 dark:border-gray-600 rounded-lg" />
                                    </div>
                                    <div class="form-control w-full md:w-1/3 flex flex-col justify-end">
                                        ...
                                        <label class="label cursor-pointer w-48 ml-auto">
                                            <span class="label-text text-gray-800 dark:text-gray-200">
                                                <LazyIcon v-if="!expandAllChecked" class="dark:text-white" name="fa:expand" />
                                                <LazyIcon v-else class="dark:text-white" name="fa:compress" />
                        &nbsp;
                                            ...
                                            <input v-model="expandAllChecked" type="checkbox" class="toggle toggle-primary">
                                        </label>
                                    </div>
                                </div>
                                <div class="overflow-x-auto w-full" style="overflow-y:hidden;">
                                    <ais-state-results>
                                        <template #default="{ results: { hits, query } }">
                                            <ais-hits v-if="hits.length > 0" class="">
                                                <template #default="{ items }">
                                                    <SearchHitsComp :items="items" :view-type-checked="viewTypeChecked"
